...
def pair_series(a: list[list], b: list[list], op: str = "ratio",
    ...
    op 為 'ratio'（a／b，如金銀比）或 'spread'（a−b，如 Brent-WTI 價差）。
    ...
    **兩種算法對非正值的處理刻意不同**：
    ...
    - spread 不做這個過濾。Brent−WTI 在 2020-04-20 的**減數是負的**
      （WTI 當日 −36.98，真實成交價），正好會踩中上面那條規則；
      但當天的價差 +54.34 是那場事件的核心事實，濾掉等於竄改歷史。
      減法本來也不會因為負數而爆掉。
    """
