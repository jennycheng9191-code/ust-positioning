...
def test_pair_series_spread_keeps_negative_wti():
    ...
    站上的算式是 Brent − WTI，而 WTI 在 2020-04-20 的 −36.98 是真實成交價——
    也就是說**減數本身是負的**，正好踩在比值那條「b ≤ 0 就跳過」的規則上。
    ...
    out = derive.pair_series(brent, wti, "spread")
    check("負油價那天留下來", [d for d, _ in out],
          ["2020-04-17", "2020-04-20", "2020-04-21"])
    check("減數為負仍算得對", out[1][1], 54.34)
    # 同一組資料走比值就會被濾掉——這正是兩種算法必須分開的理由
    check("同樣的資料在比值下會被跳過",
          [d for d, _ in derive.pair_series(brent, wti, "ratio")],
