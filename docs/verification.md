...
第 5 條（2026-08-29 隨 M5 金銀比加入）是**比值合理性**：序列存在、
比值與區間上下緣都是有限正數、最新值落在自己的近三年區間之內、
overlay 指到的價格序列確實存在。比值是相除出來的，分母若拿到零或空值
會算出 `inf`／`nan`，`derive.ratio_series()` 已在來源端跳過，這裡是最後一道網。
